...
//        }
//    }
//    class func getServicesType (completion: @escaping (_ error : Error?, _ service_Type: [servicesType])->Void) {
//
//
//
//
...
//        }
//    }
    class func firstRegister(phone: String,email: String,name:String,birthdate:String, completion: @escaping (_ error : Error?, _ success: Bool)->Void) {
             let url = "http://order.stita.ws/api/register"
             let parameters = ["name" :  name,
        "phone" :  phone,
        "email" :  email,
        "birthdate" :  birthdate]
...
//                                }
     ...
     class func confirmRegister(code: String, completion: @escaping (_ error : Error?, _ success: Bool)->Void) {
                 let url = "http://order.stita.ws/api/register"
                guard let email = helper.getEmail() else {
                    completion ( nil , false )
                    return
                }
                guard let phone = helper.getPhone() else {
                    completion ( nil , false )
                    return
                }
                guard let birthdate = helper.getBirthDate() else {
                    completion ( nil , false )
                    return
                }
                guard let name = helper.getName() else {
                    completion ( nil , false )
                    return
                }
                 let parameters = ["name" :  name,
     ...
     class func firstLogin(phone: String, completion: @escaping (_ error : Error?, _ success: Bool)->Void) {
                 let url = "http://order.stita.ws/api/login"
    ...
    class func getUserData (code: String, completion: @escaping (_ error : Error?, _ user_list: userData?)->Void) {
      ...
      class func getFood ( completion: @escaping (_ error : Error?, _ food_list: foodCollection?)->Void) {
            
            
            
            
             let url = "http://order.stita.ws/api/categories?with=foods.extras;foods.extras.extraGroup;foods.category"
                ...
                .responseJSON { responce in
                    ...
                    case.success(let value):
                      ...
                      print(value)
                                           ...
                                           guard let data = json["data"].dictionary  else  {return}
                      let food = foodCollection()
                      food.id = data["id"]?.int ?? 0
                                           food.name = json["name"].string ?? ""
                      food.description1 = json["description"].string ?? ""
                      food.has_media = json["has_media"].bool ?? true
...
//    class func getCategory ( completion: @escaping (_ error : Error?, _ category_list: [foodCatalog])->Void) {
//
//
//
//
            ...
            AF.request(url, method: .get, parameters: nil, encoding: URLEncoding.default, headers: nil)
                ...
                .responseJSON { responce in
                    ...
                    case.success(let value):
                        ...
                        var category_list = [foodCatalog]()
                        for serviceJSON in dataArr {
                            guard let service = serviceJSON.dictionary else {return}
                            let serviceObject = foodCatalog(id: (service["id"]?.int),
                                                          name: (service["name"]?.string),
                                                         
                                                          description1: (service["description"]?.string),
                                                           
                                                         
                                                          foods: service["foods"]?.arrayObject,
                                                           mediaArray: service["media"]?.arrayObject)
    
                            category_list.append(serviceObject)
                        }
                        completion(nil, category_list)
...
//                      user.birthdate = data["birthdate"]?.string ?? ""
//
//                     user_list.append(user)
//
//                 }
